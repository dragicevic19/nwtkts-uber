...
public class PrivateRideControllerIntegrationTest {
    ...
    @Test
    @DisplayName("Should return 403 when logged in user is not client")
    ...
    void shouldReturnCanceledRide() {
        ...
        HttpEntity<RideRequest> request = new HttpEntity<RideRequest>(rideRequest, headers);

        ResponseEntity<RideDTO> responseEntity =
                restTemplate.exchange("/ride/newRideRequest", HttpMethod.POST, request, RideDTO.class);

        RideDTO ride = responseEntity.getBody();
        Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        Assertions.assertEquals(RideStatus.CANCELED, ride.getRideStatus());
    }


    @Test
    @DisplayName("Should throw BadRequestException when client doesn't have enough tokens")
    void shouldThrowBadRequestException() throws Exception {
        Client client = clientRepository.findById(4L).orElseThrow(NotFoundException::new); // user@gmail.com
        client.setTokens(0.);
        clientRepository.save(client);
        ...
        mockMvc.perform(MockMvcRequestBuilders
                        ...
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(content().string("You don't have enough tokens. Buy more!"));
    }

    @Test
    @DisplayName("Should throw BadRequestException when client is already on ride")
    void shouldThrowBadRequestExceptionIfClientIsOnStartedRide() throws Exception {
        Client client = clientRepository.findById(4L).orElseThrow(NotFoundException::new); // user@gmail.com
        RideRequest rideRequest = makeRideRequest(false, false);
        makeRideForClient(client, rideRequest);
        ...
        mockMvc.perform(MockMvcRequestBuilders
                        ...
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(content().string("You can't request new ride before this one ends!"));
    }

    @Test
    @DisplayName("Should return TO_PICKUP ride status when there is active and available driver")
    void shouldReturnToPickupRide() {

        Driver activeDriver = setUpDriver(2L, true, true);

        RideRequest rideRequest = makeRideRequest(false, false);

        HttpEntity<RideRequest> request = new HttpEntity<>(rideRequest, headers);
        ...
        RideDTO ride = responseEntity.getBody();
        Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        Assertions.assertEquals(RideStatus.TO_PICKUP, ride.getRideStatus());
        Assertions.assertEquals(activeDriver.getId(), ride.getDriverId());
    }

    @Test
    @DisplayName("Should return SCHEDULED ride status WITHOUT driver when there is scheduled time (+3h) in ride request")
    void shouldReturnScheduledRide() {
        Driver activeDriver = setUpDriver(2L, true, true);

        RideRequest rideRequest = makeScheduledIn3HoursRideRequest();
        ...
        RideDTO ride = responseEntity.getBody();
        Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        Assertions.assertEquals(RideStatus.SCHEDULED, ride.getRideStatus());
        Assertions.assertNull(ride.getDriverId());
    }



    @Test
    @DisplayName("Should return SCHEDULED ride status WITH driver when there is scheduled time (+20min) in ride request")
    void shouldReturnScheduledRideWithDriver() {
        Driver activeDriver = setUpDriver(2L, true, true);
        RideRequest rideRequest = makeScheduledIn20MinRideRequest();
    ...
    @Test
    @DisplayName("Should return WAITING_FOR_PAYMENT ride status when there is friends in split fare ride that needs to pay")
    void shouldReturnWaitingForPaymentRide() {
        Driver activeDriver = setUpDriver(2L, true, true);
        RideRequest rideRequest = makeSplitFareRideRequest();
        ...
        RideDTO ride = responseEntity.getBody();
        Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        Assertions.assertEquals(RideStatus.WAITING_FOR_PAYMENT, ride.getRideStatus());
        Assertions.assertNull(ride.getDriverId());
    }

    @Test
    @DisplayName("Should return WAITING_FOR_DRIVER_TO_FINISH ride status when there is no available drivers but driver is available for next ride")
    void shouldReturnWaitingForDriverToFinishRide() {
        Driver activeDriver = setUpDriver(2L, true, false);

        Client client = this.clientRepository.findSummaryByEmail("user@gmail.com");
        Double tokensBeforeRide = client.getTokens();

        RideRequest rideRequest = makeRideRequest(false, false);
        makeRideForDriver(activeDriver, rideRequest);
        ...
        RideDTO ride = responseEntity.getBody();
        activeDriver = this.driverRepository.findById(2L).orElseThrow(NotFoundException::new);
        client = this.clientRepository.findSummaryByEmail("user@gmail.com");

        Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        Assertions.assertEquals(RideStatus.WAITING_FOR_DRIVER_TO_FINISH, ride.getRideStatus());
        Assertions.assertEquals(activeDriver.getId(), ride.getDriverId());
        Assertions.assertEquals(ride.getId(), activeDriver.getNextRideId());
        Assertions.assertEquals(tokensBeforeRide - rideRequest.getPricePerPerson(), client.getTokens());
    }

    @Test
    @DisplayName("Should return CANCELED ride status when there is no available drivers but also driver is not available for next ride")
    void shouldReturnCanceledWhenThereIsNoDriverAvailable() {
        Driver activeDriver = setUpDriver(2L, true, false);
        activeDriver.setNextRideId(2L); // busy for the next ride too
        activeDriver = this.driverRepository.save(activeDriver);
        ...
        Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        Assertions.assertEquals(RideStatus.CANCELED, ride.getRideStatus());
        Assertions.assertEquals(tokensBeforeRide, client.getTokens());
        Assertions.assertNull(ride.getDriverId());
    }

    @Test
    @DisplayName("Should return TO_PICKUP ride status with available driver when there is 2 active driver but only 1 is available")
    void shouldReturnToPickupWhenThereIsOneAvailableDriver() {
        Driver activeDriver = setUpDriver(2L, true, false);
        Driver secondDriver = setUpDriver(3L, true, true);
        ...
        Assertions.assertEquals(RideStatus.TO_PICKUP, ride.getRideStatus());
        Assertions.assertEquals(tokensBeforeRide - rideRequest.getPricePerPerson(), client.getTokens());
        Assertions.assertEquals(secondDriver.getId(), ride.getDriverId());
    }

    @Test
    @DisplayName("Should return TO_PICKUP ride status with closest driver when there is more available drivers")
    void shouldReturnToPickupWhenThereIsMoreAvailableDriver() {
        Driver activeDriver = setUpDriver(2L, true, true);
        Driver secondDriver = setUpDriver(3L, true, true);   // second driver is closer to pickup location
    ...
    @Test
    @DisplayName("Should return WAITING_FOR_DRIVER_TO_FINISH ride status with driver closest to destination when all drivers are available only for next ride")
    void shouldReturnWaitingForDriverWhenThereIsAvailableDriverForNextRide() {
        Driver activeDriver = setUpDriver(2L, true, false);    // this driver is closer to destination
        Driver secondDriver = setUpDriver(3L, true, false);
        ...
        makeRideForDriver(activeDriver, rideRequest);
        makeRideForDriver(secondDriver, rideRequest);
    ...
    @Test
    @DisplayName("Should return CANCELED ride status when driver has more than 8 working hours")
    void shouldReturnCanceledWhenThereIsAvailableDriverWithMoreThen8WorkingHours() {
        Driver activeDriver = this.driverRepository.findDetailedByEmail("driver@gmail.com");
        activeDriver.setActive(true);
        activeDriver.setAvailable(true);
        LocalDateTime login = LocalDateTime.now().minusHours(8).minusMinutes(5);
        activeDriver.getActivities().add(new DriverActivity(login, LocalDateTime.now()));
        activeDriver = this.driverRepository.save(activeDriver);
    ...
    @Test
    @DisplayName("Should return TO_PICKUP ride status when driver has less than 8 working hours")
    void shouldReturnToPickupWhenThereIsAvailableDriverWithLessThen8WorkingHours() {
        Driver activeDriver = this.driverRepository.findDetailedByEmail("driver@gmail.com");
        activeDriver.setActive(true);
        activeDriver.setAvailable(true);
        LocalDateTime login = LocalDateTime.now().minusHours(7).minusMinutes(50);
        activeDriver.getActivities().add(new DriverActivity(login, LocalDateTime.now()));
        activeDriver = this.driverRepository.save(activeDriver);
    ...
    @Test
    @DisplayName("Should return CANCELED ride status when driver is not compatible with ride requests (babies, pets, vehicle type)")
    void shouldReturnCanceledWhenDriverIsNotCompatible() {
        Driver activeDriver = setUpDriver(2L, true, true);
    ...
    private void makeRideForDriver(Driver activeDriver, RideRequest rideRequest) {
        Ride ride = new Ride(rideRequest, 1.3, rideRequest.getVehicleType());
        ride.setDriver(activeDriver);
        ride.setVehicle(activeDriver.getVehicle());
        ride.setRideStatus(RideStatus.STARTED);
        this.rideRepository.save(ride);
    }

    private void makeRideForClient(Client client, RideRequest rideRequest) {
        Ride ride = new Ride(rideRequest, 1.3, rideRequest.getVehicleType());
        ride.setRideStatus(RideStatus.STARTED);
    ...
    private RideRequest makeSplitFareRideRequest() {
        RideRequest rideRequest = makeRideRequest(false, false);
        rideRequest.setAddedFriends(List.of(new UserProfile(5L))); // seconduser@gmail.com
        return rideRequest;
    }

    private RideRequest makeScheduledIn20MinRideRequest() {
        RideRequest rideRequest = makeRideRequest(false, false);
        LocalDateTime in20Min = LocalDateTime.now().plusMinutes(20);

        rideRequest.setScheduled(in20Min.getHour() + ":" + in20Min.getMinute());
        return rideRequest;
    }

    private RideRequest makeScheduledIn3HoursRideRequest() {
        RideRequest rideRequest = makeRideRequest(false, false);
        LocalDateTime inThreeH = LocalDateTime.now().plusHours(3);

        rideRequest.setScheduled(inThreeH.getHour() + ":" + inThreeH.getMinute());
        return rideRequest;
    }
