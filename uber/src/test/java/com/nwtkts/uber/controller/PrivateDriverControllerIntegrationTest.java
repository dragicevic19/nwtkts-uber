...
public class PrivateDriverControllerIntegrationTest {
    ...
    @Test
    @DisplayName("Should return 401 when driver is not logged in")
    void shouldReturn404() throws Exception {
        RideCancelationDTO dto = makeRideCancelationDTO(1L);

        mockMvc.perform(MockMvcRequestBuilders
                        .put("/driver/cancelRide")
    ...
    void shouldThrowNotFoundException() throws Exception {
        ...
        if (rideOptional.isPresent()) {
            System.out.println("--- DELETING RIDE ---");
    ...
    @DisplayName("Should throw BadRequestException when there is no ride with that id.")
    void shouldThrowBadRequestException(RideStatus rideStatus) throws Exception {

        // create ride with certan ride status
        Long rideId = makeRide(rideStatus);

        RideCancelationDTO dto = makeRideCancelationDTO(rideId);
        ...
        mockMvc.perform(MockMvcRequestBuilders
                ...
                .andExpect(content().string("Ride is not in progress, so it can't be canceled!"));

    }

    @Test
    @DisplayName("Should throw BadRequest when driver doesnt have that ride.")
    void shouldThrowBadRequestForWrongDriver() throws Exception {
        Driver activeDriver = setUpDriver(3L, true, false);     // logged in driver has id 2L, so 3L whill cause BadRequest which is aserted
        Long rideId = makeRideForDriver(activeDriver, RideStatus.TO_PICKUP);
        ...
        mockMvc.perform(MockMvcRequestBuilders
                        ...
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(content().string("Wrong ride for driver"));

    }

    @Test
    @DisplayName("Should set driver and vehicle to null when status is scheduled.")
    void shouldSetDriverAndVehicleToNull() throws Exception {
        Driver activeDriver = setUpDriver(2L, true, false);
        Long rideId = makeRideForDriver(activeDriver, RideStatus.SCHEDULED);
        ...
        Assertions.assertEquals(ride.getDriver(), null);
        Assertions.assertEquals(ride.getVehicle(), null);

    }

    @Test
    @DisplayName("Should cancel ride when status is to pickup.")
    void shouldCancelRideWithRideStatusToPickup() throws Exception {
        Driver activeDriver = setUpDriver(2L, true, false);
        Long rideId = makeRideForDriver(activeDriver, RideStatus.TO_PICKUP);

        String cancelationReason = "Some cancelation reason.";
        RideCancelationDTO dto = makeRideCancelationDTO(rideId, cancelationReason);
        ...
        Assertions.assertEquals(ride.getRideStatus(), RideStatus.CANCELING);
        Assertions.assertEquals(ride.getCancellationReason(), cancelationReason);

    }

    @Test
    @DisplayName("Should cancel ride when status is waiting for client.")
    void shouldCancelRideWithRideStatusWaitingForClient() throws Exception {
        Driver activeDriver = setUpDriver(2L, true, false);
        Long rideId = makeRideForDriver(activeDriver, RideStatus.WAITING_FOR_CLIENT);
    ...
    private RideCancelationDTO makeRideCancelationDTO(Long rideId) {
        return new RideCancelationDTO(rideId, "Some cancelation reason.");
    }

    private RideCancelationDTO makeRideCancelationDTO(Long rideId, String cancelationReason) {
        return new RideCancelationDTO(rideId, cancelationReason);
    }

    private Long makeRide(RideStatus rideStatus) {
    ...
    private Long makeRideForDriver(Driver activeDriver, RideStatus rideStatus) {
        RideRequest rideRequest = makeRideRequest(false, false);
        Ride ride = new Ride(rideRequest, 1.3, rideRequest.getVehicleType());
        ride.setRideStatus(rideStatus);
