...
public class ReportDriverIntegrationTests {
    ...
    private void loginAsAdmin() {
        ResponseEntity<UserTokenState> responseEntity =
                restTemplate.postForEntity("/auth/login",
                        new JwtAuthenticationRequest("test@gmail.com", "123"),
                        UserTokenState.class);
        UserTokenState userTokenState = responseEntity.getBody();
        adminAccessToken = "Bearer " + userTokenState.getAccessToken();
    }
    ...
    @Test
    @DisplayName("Should throw ForbiddenException when admin wants to report driver.")
    void shouldThrowForbiddenIfAdminWantsToReportDriver() throws Exception {
        loginAsAdmin();
    ...
    @Test
    @DisplayName("Happy path: Should return OK:200 when Driver is reported with correct input parameters")
