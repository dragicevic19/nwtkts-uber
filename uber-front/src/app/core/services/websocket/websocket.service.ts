...
export class WebsocketService {
  ...
  openGlobalSocket() {
    ...
    this.stompClient.subscribe('/map-updates/client-notifications-scheduled-ride-in',
      (message: { body: string }) => {
        const notification: ClientNotification = JSON.parse(message.body);
    ...
    this.stompClient.subscribe('/map-updates/new-ride-for-driver',
      (message: { body: string }) => {
        const ride: DriversRide = JSON.parse(message.body);
    ...
    this.stompClient.subscribe('/map-updates/split-fare-change-status',
      (message: { body: string }) => {
        const ride: ClientsSplitFareRide = JSON.parse(message.body);
